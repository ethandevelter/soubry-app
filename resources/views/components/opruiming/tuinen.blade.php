...
<div class="bg-gray-50 font-sans">
            ...
            <div class="flex flex-wrap justify-between items-start">
                ...
                <div class="w-full md:w-7/12 flex flex-wrap space-y-6 md:space-y-0">
                    ...
                    @php
                        ...
                        $sortedRecords = $records->sortByDesc(fn($cat) => $cat->cat === $tuin->slug);
                    @endphp
                    @foreach ($sortedRecords as $cat)
                        <div class="w-full md:w-1/2 mb-12">
                            <h3 class="text-xl @if($tuin->slug == $cat->cat) text-mainhomeone font-semibold @else text-mainhometwo font-medium @endif mb-4">{{ $cat->title }}</h3>
    ...
    <!-- Our Process Section -->
    <section class="pb-32">
        <x-steps :block="$tuin"></x-steps>
    </section>
    @if($tuin->images)
    <section class="py-20">
        <x-container>
            <h2 class="text-black text-center my-24">{{$tuin->fourth_title}}</h2>
            <div class="flex flex-wrap -mr-6">
                @php
                    $imagePaths = json_decode($tuin->images);
                    shuffle($imagePaths);
                @endphp
