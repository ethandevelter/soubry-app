@php
    $overlijden = table('overlijdenpage')->first();
@endphp
<section class="bg-gray-50">
    <div class="relative flex flex-col items-center justify-center text-center py-44 bg-cover bg-center hero-overlay min-h-60vh" style="background-image : url({{asset($overlijden->hero_image)}})">
        <h1 class="alt">{{$overlijden->intro_title}}</h1>
        <div class="mt-6 max-w-lg mx-auto main-text text-white z-10 !font-extralight">{{$overlijden->intro_text}}</div>
    </div>
</section>
        ...
        <x-container>
            <div class="flex flex-wrap justify-between items-start">
                ...
                <div class="w-full md:w-5/12 mb-6 md:mb-0">
                    <h2 class="text-black mb-4">{{$overlijden->second_title}}</h2>
                    <div class="main-text !text-base text-black mb-6 w-9/12">{{$overlijden->second_text}}</div>
                    <a href="/contact" class="btn-white alt">{{$overlijden->button_text}} <span><x-map-arrowwatcher></x-map-arrowwatcher></span></a>
                </div>
    ...
    <section class="py-20">
        <x-container>
            ...
            <div class="flex flex-wrap -mr-6">
                @php
                    $imagePaths = json_decode($overlijden->images);
                    shuffle($imagePaths);
                @endphp
