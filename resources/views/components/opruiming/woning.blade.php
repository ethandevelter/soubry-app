        ...
        <x-container>
            <div class="flex flex-wrap justify-between items-start">
                ...
                <div class="w-full md:w-5/12 mb-6 md:mb-0">
                    ...
                    <div class="main-text !text-base text-black mb-6 w-9/12">{{$woning->second_text}}</div>
                    <a href="/contact" class="btn-white alt">{{$woning->button_text}} <span><x-map-arrowwatcher></x-map-arrowwatcher></span></a>
                </div>
                ...
                <div class="w-full md:w-7/12 flex flex-wrap space-y-6 md:space-y-0">
                    ...
                    @php
                        ...
                        $sortedRecords = $records->sortByDesc(fn($cat) => $cat->cat === $woning->slug);
                    @endphp
                    @foreach ($sortedRecords as $cat)
                        <div class="w-full md:w-1/2 mb-12">
                            <h3 class="text-xl @if($woning->slug == $cat->cat) text-mainhomeone font-semibold @else text-mainhometwo font-medium @endif mb-4">{{ $cat->title }}</h3>
            ...
            <div class="flex flex-row pt-32">
                <div class="w-8/12 text-black main-text !text-base"> {!! $woning->extra_text !!}</div>
